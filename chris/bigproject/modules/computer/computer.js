...
.controller('ViewComputerCtrl', ['$scope', function($scope) {
...
}]).controller('ViewComputerCreateCtrl', ['$scope', '$location', 'dataService', function($scope,$location, dataService) {
    //location starts 
    $scope.mapOptions = { 
        types: 'geocode' 
    }; 
  ...
  function addSuccess(response) {
    ...
    $scope.frm = {};
    $location.path('/computer/create/images/'+response.data.data.id);
  }
  ...
  function addFailure(response) {
    ...
    $scope.submitCreateForm = function() {
        //url
        var url = 'http://bootstrap.mkgalaxy.com/svnprojects/horo/records.php?action=add&saveIP=1&access_token='+$scope.loggedInUsersData.token+'&path=/chris/computer&tid=1';
        console.log(url);
        //postdata
        var postData = '';
...
}]).controller('ViewComputerImagesCtrl', ['$scope', '$location', 'dataService', '$routeParams', function($scope, $location, dataService, $routeParams) {
