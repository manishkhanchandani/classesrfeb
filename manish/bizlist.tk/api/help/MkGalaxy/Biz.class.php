...
class Biz
{
  ...
  function saveDataResults($res) {
    ...
    foreach ($res as $v) {
      $d = array();
      $d['id'] = guid();
      $d['lat'] = $v['geometry']['location']['lat'];
      $d['lng'] = $v['geometry']['location']['lng'];
      $d['place_id'] = $v['place_id'];
      $d['name'] = $v['name'];
      $d['data'] = json_encode($v);
      $d['url'] = url_name_v2($v['name']).'/'.$v['place_id'];

      $query = "select * from bizlist_business where place_id = ?";
      $check = $modelGeneral->fetchRow($query, array($d['place_id']), 0);
      if (!empty($check)) {
        continue;
      }//end if

      $modelGeneral->addDetails('bizlist_business', $d);
    }
  ...
  function saveDetailResults($res) {
    if (empty($res)) {
      return false;
    }
  ...
  public function detailBiz($id, $cacheTime=900) {
    global $modelGeneral;
    $query = "select * from bizlist_business as m WHERE m.place_id = ? or m.id = ?";
    $data = $modelGeneral->fetchRow($query, array($id, $id), $cacheTime);
    return $data;
  }//end detail
