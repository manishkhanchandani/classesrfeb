...
class repertory_Complete
{
  ...
  public function getAllSearch($Models_General, $keyword, $max=100, $page=0, $cacheTime=0)
  {
    ...
    $query_rsView = 'select a.*, b.id as bid, b.path as bpath from '.self::$tableCompleteRepertory.' as a LEFT JOIN '.self::$tableCompleteRepertory.' as b ON a.chapter = b.id WHERE a.remedies is NOT NULL '.$string.' order by b.id ASC, a.path ASC';
    $query_limit_rsView = sprintf("%s LIMIT %d, %d", $query_rsView, $startRow_rsView, $maxRows_rsView);
    $results = $Models_General->fetchAll($query_limit_rsView, array(), $cacheTime);
    if (!empty($results)) {
      foreach ($results as $k => $v) {
  ...
  public function browseByChapter($Models_General, $chapter, $max=100, $page=0, $cacheTime=0)
  {
    ...
    if ($chapter == 0) {
      $query_rsView = 'select a.*, b.id as bid, b.path as bpath from '.self::$tableCompleteRepertory.' as a LEFT JOIN '.self::$tableCompleteRepertory.' as b ON a.chapter = b.id WHERE a.remedies is NOT NULL order by b.id ASC, a.path ASC';
    } else {
      $query_rsView = sprintf('select a.*, b.id as bid, b.path as bpath from '.self::$tableCompleteRepertory.' as a LEFT JOIN '.self::$tableCompleteRepertory.' as b ON a.chapter = b.id WHERE a.remedies is NOT NULL AND a.chapter = %s order by b.id ASC, a.path ASC', GetSQLValueString($chapter, 'int'));
    }
    $query_limit_rsView = sprintf("%s LIMIT %d, %d", $query_rsView, $startRow_rsView, $maxRows_rsView);
    $results = $Models_General->fetchAll($query_limit_rsView, array(), $cacheTime);
    if (!empty($results)) {
    ...
    $return['sql1'] = $sql1;
    if ($chapter == 0) {
      $queryTotalRows = 'select count(*) as cnt from '.self::$tableCompleteRepertory.' as a LEFT JOIN '.self::$tableCompleteRepertory.' as b ON a.chapter = b.id WHERE a.remedies is NOT NULL';
    } else {
